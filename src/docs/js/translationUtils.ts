...
type RouteTranslationsMap = Record<LocaleCode, Record<string, string>>;
type LocalizedCollectionsMap = Record<string, Record<LocaleCode, string>>;

const routeTranslationsMap = routeTranslations as RouteTranslationsMap;
const localizedCollectionsMap = localizedCollections as LocalizedCollectionsMap;
...
export async function getLocalizedPathname(
  ...
  let oldPath: string, currLocale: LocaleCode;
...
export async function generateRouteTranslations() {
  ...
  const collections = Object.keys(localizedCollectionsMap) as Array<keyof DataEntryMap>;
  ...
  const dynamicRouteTranslations: Record<string, Record<string, string>> = Object.fromEntries(
    locales.map((locale) => [locale, { ...routeTranslationsMap[locale] }]),
  );
  ...
  allContent.forEach((entry) => {
    ...
    const base = localizedCollectionsMap[entry.collection]?.[locale as LocaleCode] ?? entry.collection;
    ...
    if (mappingKey) {
      ...
      otherLocales.forEach((otherLocale: string) => {
        const otherBase =
          localizedCollectionsMap[entry.collection]?.[otherLocale as LocaleCode] ?? entry.collection;
